...
class StartScreen(wx.Frame):
    ...
    def create_button(self, label, event, **kwargs):
        button = wx.Button(self.main_panel, label=label)
        button.Bind(wx.EVT_BUTTON, event)
        return button
    
    def start_ui(self):
        self.pacs_connection_button = self.create_button('PACS Connection', self.on_pacs_connection)
    ...
    def on_pacs_connection(self, event):
